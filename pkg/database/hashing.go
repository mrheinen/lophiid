...
func GetSameRequestHash(req *models.Request) (string, error) {
	...
	for _, header := range req.Headers {
		...
		if len(headerArray) == 2 {
			if strings.ToLower(headerArray[0]) == "host" || strings.ToLower(headerArray[0]) == "user-agent" || strings.ToLower(headerArray[0]) == "referer" || strings.ToLower(headerArray[0]) == "content-length" {
				continue
			}
