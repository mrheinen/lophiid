...
func TestGetSameRequestHash(t *testing.T) {
	tests := []struct {
		...
		{
			name: "fortinet logincheck with different credentials should match",
			req1: models.Request{
				Method:      "POST",
				Path:        "/remote/logincheck",
				Headers:     pgtype.FlatArray[string]{"Host: 185.93.221.187:443", "Accept: */*", "Accept-Encoding: gzip", "Content-Length: 47", "Content-Type: application/x-www-form-urlencoded", "User-Agent: Mozilla/5.0"},
				Body:        []byte("ajax=1&username=frisco&realm=&credential=frisco"),
				ContentType: "application/x-www-form-urlencoded",
			},
			req2: models.Request{
				Method:      "POST",
				Path:        "/remote/logincheck",
				Headers:     pgtype.FlatArray[string]{"Host: 209.182.225.190:443", "Accept: */*", "Accept-Encoding: gzip", "Content-Length: 56", "Content-Type: application/x-www-form-urlencoded", "User-Agent: Mozilla/5.0"},
				Body:        []byte("ajax=1&username=saoraimundo&realm=&credential=qwertyuiop"),
				ContentType: "application/x-www-form-urlencoded",
			},
			shouldMatch: true,
			wantErr:     false,
