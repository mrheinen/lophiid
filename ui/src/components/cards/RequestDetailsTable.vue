  ...
  <table>
    <tbody>
      ...
      <tr>
        ...
        <td>
          <a
            :href="'/requests?q=honeypot_ip:' + request.honeypot_ip"
          >
            {{ request.honeypot_ip }}
          </a>
        </td>
      ...
      <tr v-if="request.p0f_result && (this.calculatedUptimeDays > 0 ||
  ...
  watch: {
    request() {
      if (this.request.p0f_result) {
        this.calculatedUptimeDays = Math.round(this.request.p0f_result.uptime_minutes / (60 * 24));
        this.calculatedUptimeAndHours = Math.round((this.request.p0f_result.uptime_minutes % (60*24)) / 60);
      }
    }
  },
}
